...
- A check named in [`ci.decision_checks`](/no-mistakes/reference/repo-config/#cidecision_checks) never reaches the fix agent. Its red state means a person has a decision outstanding, not that something is broken, so when one is failing the step parks at an ask-user gate naming it before any fix round runs, its logs are never fetched, and it is never offered as a fix target. That gate offers no fix round and re-parks even under an explicit fix response: the declaration lives on the repository's own trusted default branch and a run-time answer does not dissolve it
- A proposed CI repair is refused, uncommitted, when its effect is to undo work the branch deliberately did: a file it returns to byte-identical pre-branch content, or a line that exists exactly once in the pre-branch file, that the branch removed and the repair restores. Both are measured against the fork point with git alone, before anything is staged, so the branch head and the run state are untouched and the proposed changes stay in the run worktree. The step parks with the evidence. Answering the gate with a fix selection authorises exactly the listed reversion and nothing else: that response runs another repair round rather than committing the one you read, so a round that would undo anything different - wider or narrower - parks again with its own evidence, and a finding too large to list in full cannot be authorised at the gate at all. This exists because a repair that undoes the branch's own work is invisible in the branch's `base..head` diff, so nothing downstream - including the re-review that follows every CI repair - can see it
